...
const CalendarColumn = () => {
    ...
    return (
      <div className="mx-12 w-96">
        ...
        <h2 className="text-4xl text-[#738755]">ACTIVIDADES</h2>
        ...
        <div className="mb-6 flex justify-between items-center">
          ...
          <h3 className="text-2xl text-[#957F74]">ABRIL 2024</h3>
          <ChevronRight className="w-8 h-8 text-[#957F74] cursor-pointer" />
        </div>
        
        <div className="grid grid-cols-7 gap-2">
          {weekDays.map(day => (
            <div key={day} className="text-center text-base text-[#957F74] py-3">
              {day}
            </div>
          ))}
          {Array.from({ length: 30 }).map((_, i) => (
            <div
              key={i}
              className="text-center py-3 text-base transition hover:bg-green-100 cursor-pointer rounded"
            >
              {i + 1}
            </div>
          ))}
